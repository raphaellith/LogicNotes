...
A \emph{language} refers to a set \(L\) of strings over a finite alphabet \(\Sigma\).
%
\[L \subseteq \Sigma^{*}\]
...
Moreover, a language \(L\) is \emph{recursively enumerable} (abbreviated \emph{r.e.}, or alternatively \emph{semi-decidable}) if there exists a computer program that outputs strings from \(L\), only strings from \(L\), and will eventually output any given string from \(L\).
...
If a language \(L \subseteq \Sigma^{*}\) is recursively enumerable, then its complement \(\Sigma^{*}\setminus L\) is said to be \emph{co-recursively enumerable} (abbreviated \emph{co-r.e.}).


\begin{theorem}
    If a set is both recursively enumerable and co-recursively enumerable, then it is decidable.
\end{theorem}
\begin{proof}
    Let \(L\) be a language that is both recursively enumerable and co-recursively enumerable.
