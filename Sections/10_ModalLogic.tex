...
\end{itemize}
%
Figure \ref{fig:Ch10-frame-p-morphism} shows an example of a frame p-morphism between two frames \((W, R)\) and \((W', R')\), with a function \(f\) mapping worlds from \(W = \{A, B, C, D, E\}\) to worlds from \(W' = \{X, Y, Z\}\). Notice that
%
\begin{itemize}
